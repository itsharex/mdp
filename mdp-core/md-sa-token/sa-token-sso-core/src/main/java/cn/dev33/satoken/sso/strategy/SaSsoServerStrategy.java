...
public class SaSsoServerStrategy {
     ...
     * 发送 Http 请求的处理函数
     */
    private SendRequestFunction sendRequest = url -> {
     ...
     * 使用异步模式执行一个任务
     */
    private SaParamFunction<SaFunction> asyncRun = fun -> {
        new Thread(() -> {
            fun.run();
        }).start();
    };
     ...
     * 未登录时返回的 View
     */
    private NotLoginViewFunction notLoginView = () -> {
     ...
     * SSO-Server端：登录函数
     */
    private DoLoginHandleFunction doLoginHandle = (name, pwd) -> {
     ...
     */
    private SaParamFunction<String> jumpToRedirectUrlNotice = (redirectUrl) -> {
     ...
     */
    private CheckTicketAppendDataFunction checkTicketAppendData = (loginId, result) -> {
        return result;
    };
     ...
     * @return 返回的结果
     */
    private SaResult requestAsSaResult(String url) {

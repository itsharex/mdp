...
public class SaSsoServerTemplate extends SaSsoTemplate {
     ...
     * Server 相关策略
     */
    private SaSsoServerStrategy strategy = new SaSsoServerStrategy();

    public SaSsoServerTemplate() {
        super.getMessageHolder().addHandle(new SaSsoMessageCheckTicketHandle());
        super.getMessageHolder().addHandle(new SaSsoMessageSignoutHandle());
    }
    ...
    public String buildRedirectUrl(String client, String redirect, Object loginId, String tokenValue) {
        ...
        return SaFoxUtil.joinParam(encodeBackParam(redirect), getParamName().getTicket(), ticket);
    }
    ...
    public String encodeBackParam(String url) {
        ...
        // 获取back参数所在位置
        int index = url.indexOf("?" + getParamName().getBack() + "=");
        if (index == -1) {
            index = url.indexOf("&" + getParamName().getBack() + "=");
            if (index == -1) {
                return url;
            }
        }
        ...
        // 开始编码
        int length = getParamName().getBack().length() + 2;
    ...
    public void checkAllowUrlList(List<String> allowUrlList) {
        SaSsoClientModel.checkAllowUrlListStaticMethod(allowUrlList);
    }
    ...
    public void registerSloCallbackUrl(Object loginId, String client, String sloCallbackUrl) {
        ...
        int maxRegClient = getServerConfig().getMaxRegClient();
        if (maxRegClient != -1) {
            for (; ; ) {
    ...
    public int calcNextIndex(List<SaSsoClientInfo> scmList) {
        ...
        // 获取目前最大的index值
        int maxIndex = scmList.get(scmList.size() - 1).getIndex();
    ...
    public String notifyClientLogout(Object loginId, String deviceId, SaSsoClientInfo scm, boolean autoLogout, boolean isPushWork) {
        ...
        // 如果给个null值，不进行任何操作
        if (scm == null || scm.getMode() != SaSsoConsts.SSO_MODE_3) {
            return null;
        }
        ...
        paramsMap.put(getParamName().getClient(), scm.getClient());
        paramsMap.put(getParamName().getLoginId(), loginId);
        paramsMap.put(getParamName().getDeviceId(), deviceId);
        paramsMap.put(getParamName().getAutoLogout(), autoLogout);
        ...
        // 发起请求
        return strategy.getSendRequest().apply(finalUrl);
    }
    ...
    public void pushToAllClient(SaSsoMessage message, String ignoreClient) {
        ...
        for (SaSsoClientModel client : needPushClients) {
            ...
            strategy.getAsyncRun().run(() -> pushMessage(client, message));
        }
    }
    ...
    public SaSsoServerConfig getServerConfig() {
        return SaSsoServerManager.getServerConfig();
    }

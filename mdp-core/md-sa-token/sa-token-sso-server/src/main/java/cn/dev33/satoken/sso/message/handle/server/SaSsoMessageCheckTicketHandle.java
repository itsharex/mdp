...
public class SaSsoMessageCheckTicketHandle implements SaSsoMessageHandle {
    ...
    public Object handle(SaSsoTemplate ssoTemplate, SaSsoMessage message) {
        ...
        String client = message.getString(paramName.getClient());
        String ticket = message.getValueNotNull(paramName.getTicket()).toString();
        String sloCallback = message.getString(paramName.getSsoLogoutCall());
        ...
        result.set(paramName.getLoginId(), loginId);
        result.set(paramName.getTokenValue(), ticketModel.getTokenValue());
        result.set(paramName.getDeviceId(), stpLogic.getLoginDeviceIdByToken(ticketModel.getTokenValue()));
        result.set(paramName.getRemainTokenTimeout(), stpLogic.getTokenTimeout(ticketModel.getTokenValue()));
        result.set(paramName.getRemainSessionTimeout(), stpLogic.getSessionTimeoutByLoginId(loginId));
        result = ssoServerTemplate.getStrategy().getCheckTicketAppendData().apply(loginId, result);
        return result;
    }

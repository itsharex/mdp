...
public class SaSsoClientTemplate extends SaSsoTemplate {
    ...
    @Getter
    @Setter
    private SaSsoClientStrategy strategy = new SaSsoClientStrategy();

    public SaSsoClientTemplate() {
        super.getMessageHolder().addHandle(new SaSsoMessageLogoutCallHandle());
    }
    ...
    public String buildServerAuthUrl(String clientLoginUrl, String back) {
        ...
        if (SaFoxUtil.isNotEmpty(client)) {
            serverUrl = SaFoxUtil.joinParam(serverUrl, getParamName().getClient(), client);
        }
         ...
         */
        if (!clientLoginUrl.contains(getParamName().getBack() + "=")) {
            clientLoginUrl = SaFoxUtil.joinParam(clientLoginUrl, getParamName().getBack(), back);
        }

        // 返回
        return SaFoxUtil.joinParam(serverUrl, getParamName().getRedirect(), clientLoginUrl);
    }
    ...
    public SaSsoMessage buildCheckTicketMessage(String ticket, String ssoLogoutCallUrl) {
        ...
        message.set(getParamName().getClient(), getClient());
        message.set(getParamName().getTicket(), ticket);
        message.set(getParamName().getSsoLogoutCall(), ssoLogoutCallUrl);
        return message;
    }
    ...
    public SaSsoClientConfig getClientConfig() {
        return SaSsoClientManager.getClientConfig();
    }

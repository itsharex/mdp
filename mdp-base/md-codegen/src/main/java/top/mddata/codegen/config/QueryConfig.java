...
public class QueryConfig implements Serializable {
    ...
    public String buildExtends(GlobalConfig globalConfig) {
        ...
        Class<?> varSuperClass = queryConfig.getSuperClass();
        if (varSuperClass != null) {
            String type = "";
            if (queryConfig.getSuperClassGenericity()) {
                if (queryConfig.getGenericityType() == null) {
                    type = StrUtil.format("<{}>", Long.class.getSimpleName());
                } else {
                    type = StrUtil.format("<{}>", queryConfig.getGenericityType().getSimpleName());
                }
            }
    ...
    public String buildImplements(GlobalConfig globalConfig) {
        Class<?>[] entityInterfaces = globalConfig.getQueryConfig().getImplInterfaces();

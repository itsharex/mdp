...
public class SaSsoServerConfig implements Serializable {
     ...
     * ticket 有效期 (单位: 秒)
     */
    private long ticketTimeout = 60 * 5;
     ...
     */
    private Boolean autoRenewTimeout = false;

    /**
     * 在 Account-Session 上记录 Client 信息的最高数量（-1=无限），超过此值将进行自动清退处理，先进先出
     */
    private int maxRegClient = 32;
     ...
     * Client 信息配置列表
     */
    private Map<String, SaSsoClientModel> clients = new LinkedHashMap<>();
     ...
     * 是否允许匿名 Client 接入
     */
    private Boolean allowAnonClient = false;
